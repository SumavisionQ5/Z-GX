...
//XXX: NEEDS UPDATE
// Handshake TH del lightgun Stunner (portado de Yabause saturn-wii smpc.c:690)
static u8 do_th_mode(u8 val)
{
	switch (val & 0x40) {
	case 0x40:
		return 0x70 | ((per_data.data[3] & 0xF) & 0xc);
	case 0x00:
		return 0x30 | ((per_data.data[2] >> 4) & 0xf);
	}
	return 0;
}
void FASTCALL SmpcWriteByte(u32 addr, u8 val) {
	...
	{ extern u32 zgx_gun_ddr[8], zgx_gun_lastval; zgx_gun_ddr[(SMPC_REG_DDR1 >> 5) & 7]++; zgx_gun_lastval = SMPC_REG_DDR1; }
				case 0x00: // Modo directo: si es gun y val 0x7F, devolver data[2]
					...
					break;
				case 0x40: // TH Control Method (LIGHTGUN Stunner)
