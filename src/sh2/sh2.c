...
u8 sh2_Read8(u32 addr)
{
	if ((addr & 0xFE000000) == 0x06000000) {
		extern u8 *wram;
		// FIX deadlock: si el SLAVE pollea repetidamente la misma dir de HWRAM (spin
		// esperando al master), cederle el slice al master. Ver VC2_DIAGNOSTICO_FINAL.md
		if (sh_ctx->flags & SH2_FLAG_SLAVE) {
			static u32 _last_addr = 0; static u32 _same_count = 0;
			if (addr == _last_addr) {
				if (++_same_count >= 8 && sh_ctx->cycles < 0) { sh_ctx->cycles = 0; _same_count = 0; }
			} else { _last_addr = addr; _same_count = 0; }
		}
