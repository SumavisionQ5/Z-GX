...
u8 sh2_Read8(u32 addr)
{
	if ((addr & 0xFE000000) == 0x06000000) {
		extern u8 *wram;
		// poll detect (PicoDrive style): SLAVE en spin = misma dir + mismo PC (bloque re-ejecutado)
		if (sh_ctx->flags & SH2_FLAG_SLAVE) {
			u32 ca = addr & 0x0FFFFFFF;
			if (ca == sh_ctx->poll_addr && sh_ctx->pc == sh_ctx->poll_pc) {
				if (++sh_ctx->poll_cnt >= 16) { sh_ctx->poll_state = 1; sh_ctx->cycles = 0; }
			} else { sh_ctx->poll_addr = ca; sh_ctx->poll_pc = sh_ctx->pc; sh_ctx->poll_cnt = 0; }
		}
	...
	switch(addr >> 29) {
		...
		case 0x0:	//Cache area
			// poll detect (PicoDrive): SLAVE en spin = misma dir + mismo PC (bloque re-ejecutado)
			if (sh_ctx->flags & SH2_FLAG_SLAVE) {
				u32 ca = addr & 0x0FFFFFFF;
				if ((ca & 0xFF000000) == 0x06000000) {
					if (ca == sh_ctx->poll_addr && sh_ctx->pc == sh_ctx->poll_pc) {
						if (++sh_ctx->poll_cnt >= 16) { sh_ctx->poll_state = 1; sh_ctx->cycles = 0; }
					} else { sh_ctx->poll_addr = ca; sh_ctx->poll_pc = sh_ctx->pc; sh_ctx->poll_cnt = 0; }
				}
...
u16 sh2_Read16(u32 addr)
{
	...
	if ((addr & 0xFFF00000) == 0x00200000) { extern u8 *wram; return *((u16*)(wram + (addr & 0xFFFFF))); }
	switch(addr >> 29) {
		case 0x1:	//Cache-through area
		case 0x5:	sh_ctx->cycles += mem_CyclesR(addr);
		case 0x0:	//Cache area
...
u32 sh2_Read32(u32 addr)
{
	...
	if ((addr & 0xFFF00000) == 0x00200000) { extern u8 *wram; return *((u32*)(wram + (addr & 0xFFFFF))); }
	switch(addr >> 29) {
		case 0x1:	//Cache-through area
		case 0x5:	sh_ctx->cycles += mem_CyclesR(addr);
		case 0x0:	//Cache area
...
{
	{ extern SH2 ssh2; if (ssh2.poll_state) { u32 pa = ssh2.poll_addr, wa = addr & 0x0FFFFFFF; if (wa >= pa - 32 && wa <= pa + 32) ssh2.poll_state = 0; } }
...
void sh2_Write32(u32 addr, u32 val)
{
	...
	if ((addr & 0xFFF00000) == 0x00200000) { extern u8 *wram; *((u32*)(wram + (addr & 0xFFFFF))) = val; return; }
	switch(addr >> 29) {
		case 0x1:	//Cache-through area
		case 0x5:	sh_ctx->cycles += mem_CyclesW(addr);
		case 0x0:	//Cache area
...
u16* sh2_GetPCAddr(u32 pc)
{
	...
	//}
	pc &= 0x0FFFFFFF;	//Fix: enmascarar region espejo SH2 cache-through -> evita DSI
	...
	switch((pc >> 20) & 0xFF) {
		...
		default: // Invalid
			return (u16*) wram;  //Fix: evitar DSI en dir invalida
	}
	return 0;
}
#endif
