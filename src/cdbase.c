...
/*! \file cdbase.c
    \brief Dummy and ISO, BIN/CUE, CD Interfaces
*/
#define _GNU_SOURCE
#include <string.h>
#ifndef WIN32
...
enum IMG_TYPE { IMG_NONE, IMG_ISO, IMG_BINCUE, IMG_CHD, IMG_NRG };
...
static FILE* fopenInPath(char* filename, char* path){
  ...
  while ((ent = readdir(d)) != NULL) {
    if (strncasecmp(filename, ent->d_name, l) == 0) {
      ...
      *tmp++ = '/';
      for (k=0; k<strlen(ent->d_name); k++) {
        if (shallBeEscaped(ent->d_name[k])) *tmp++='\\';
           *tmp++ = ent->d_name[k];
      }
      *tmp++ = '\0';
      closedir(d);
...
static int ISOCDReadSectorFAD(u32 FAD, void *buffer) {
   ...
   if (currentTrack->sector_size == 2448)
   {
      ...
      {
         fread(buffer, 2448, 1, currentTrack->fp);
      }
      else
      {
         ...
         fread(buffer, 2352, 1, currentTrack->fp);

         fread(subcode_buffer, 96, 1, currentTrack->fp);
         fseek(currentTrack->fp, 2352, SEEK_CUR);
         fread(subcode_buffer + 96, 96, 1, currentTrack->fp);
         fseek(currentTrack->fp, 2352, SEEK_CUR);
         fread(subcode_buffer + 192, 96, 1, currentTrack->fp);
         for (i = 0; i < 96; i++)
   ...
   else if (currentTrack->sector_size == 2048)
   {
      ...
      fread((char *)buffer + 0x10, 2048, 1, currentTrack->fp);
   }
	return 1;
}
