...
{
	BAT b = {{0}};
	b.data[0] = virtual | ((1 << (bl_size_enc + 2)) - 1);
	b.data[1] = (MEM_VIRTUAL_TO_PHYSICAL(mem)) | 0b10;
	/*
	b.BRPN = MEM_VIRTUAL_TO_PHYSICAL(mem) >> 17;
	b.WIMG = 0b0000;
	b.PP = 0b10;
	*/
	switch (bat_id) {
		case 0:
			asm volatile(	"mtspr 540, %0 \n\t"	//DBAT2
							"mtspr 541, %1 \n\t"
							:: "r" (b.data[0]), "r" (b.data[1])); break;
		case 1:
			asm volatile(	"mtspr 542, %0 \n\t"	//DBAT3
							"mtspr 543, %1 \n\t"
							:: "r" (b.data[0]), "r" (b.data[1])); break;
		case 2:
			asm volatile(	"mtspr 572, %0 \n\t"	//DBAT6
							"mtspr 573, %1 \n\t"
							:: "r" (b.data[0]), "r" (b.data[1])); break;
		case 3:
			asm volatile(	"mtspr 574, %0 \n\t"	//DBAT7
							"mtspr 575, %1 \n\t"
							:: "r" (b.data[0]), "r" (b.data[1])); break;
	}
}
...
{
	asm volatile(	"mfdbatu %0, 0 \n\t"
					"mfdbatl %1, 0"
					: "=r" (bats[0].data[0]), "=r" (bats[0].data[1])
					:);

	asm volatile(	"mfdbatu %0, 1 \n\t"
					"mfdbatl %1, 1"
					: "=r" (bats[1].data[0]), "=r" (bats[1].data[1])
					:);
	asm volatile(	"mfdbatu %0, 2 \n\t"
					"mfdbatl %1, 2"
					: "=r" (bats[2].data[0]), "=r" (bats[2].data[1])
					:);

	asm volatile(	"mfdbatu %0, 3 \n\t"
					"mfdbatl %1, 3"
					: "=r" (bats[3].data[0]), "=r" (bats[3].data[1])
					:);

	asm volatile(	"mfspr %0, 568 \n\t"
					"mfspr %1, 569"
					: "=r" (bats[4].data[0]), "=r" (bats[4].data[1])
					:);

	asm volatile(	"mfspr %0, 570 \n\t"
					"mfspr %1, 571"
					: "=r" (bats[5].data[0]), "=r" (bats[5].data[1])
					:);
	asm volatile(	"mfspr %0, 572 \n\t"
					"mfspr %1, 573"
					: "=r" (bats[6].data[0]), "=r" (bats[6].data[1])
					:);

	asm volatile(	"mfspr %0, 574 \n\t"
					"mfspr %1, 575"
					: "=r" (bats[7].data[0]), "=r" (bats[7].data[1])
					:);
...
#if 0
int VM_mmap_pte(PTE **pte_arr, void *mem, u32 virtual, size_t size)
{
	unsigned int i;
	u32 phys;
	PTE *pte;
...
int VM_MemMap(void *mem, u32 virtual, size_t size, u32 PP)
{
	...
	for (i = 0; i < size; i += PAGE_SIZE) {
		pte = insert_pte(virtual + i, phys + i, 0, PP);
		if (!pte)
			return -ENOMEM;
	}
